...
個人開発で早くリリースでき、AI エージェントが扱いやすい構成を優先する。依存関係と運用サービスは最小限にする。
...
Google OAuth の開始とセッション管理は Supabase に委譲する。Next.js は `@supabase/ssr` でセッションを読み取り、API の認可に利用する。認証情報はアプリケーションコードで扱わない。
...
詳細なテスト手順は [テスト方針](TESTING.md) を参照する。
